...
def fc_op(input_op, name, n_out, layer_collector, act_func=tf.nn.leaky_relu):
    ...
    with tf.name_scope(name) as scope:
        kernel = tf.Variable(
            tf.contrib.layers.xavier_initializer()([n_in, n_out]),
            dtype=tf.float32,
            name=scope + "w",
        )
        ...
        biases = tf.Variable(
            tf.constant(0, shape=[1, n_out], dtype=tf.float32), name=scope + "b"
        )
...
class SDNE(object):
    def __init__(
        ...
        if self.lr is None:
            self.lr = tf.train.inverse_time_decay(
                0.03, self.max_iter, decay_steps=1, decay_rate=0.9999
            )
    ...
    def train(self):
        ...
        L_1st = tf.reduce_sum(
            ...
            * (
                _embeddings_norm
                - 2 * tf.matmul(_embeddings, tf.transpose(_embeddings))
        ...
        for param in layer_collector:
            L += self.nu1 * tf.reduce_sum(tf.abs(param[0])) + self.nu2 * tf.reduce_sum(
                tf.square(param[0])
            )
        ...
        for step in range(self.max_iter):
            ...
            self.sess.run(
                train_op,
                feed_dict={
                    AdjBatch: adj_batch_train,
                    Adj: adj_mat_train,
                    B: b_mat_train,
                },
            )
            if step % 50 == 0:
                l, l1, l2 = self.sess.run(
                    (L, L_1st, L_2nd),
                    feed_dict={
                        AdjBatch: adj_batch_train,
                        Adj: adj_mat_train,
                ...
                print(
                    "step %i: total loss: %s, l1 loss: %s, l2 loss: %s"
                    % (step, l, l1, l2)
                )
...
class SDNE2(object):
    def __init__(
        ...
        if self.lr is None:
            self.lr = tf.train.inverse_time_decay(
                0.1, self.max_iter, decay_steps=1, decay_rate=0.9999
            )
    ...
    def get_train(self):
        ...
        L_1st = tf.reduce_sum(
            Weights * (tf.reduce_sum(tf.square(embs[0] - embs[1]), axis=1))
        )
        ...
        for step in range(self.max_iter + 1):
            ...
            if step % 50 == 0:
                print(
                    "step %i: %s"
                    % (step, self.sess.run([L, L_1st, L_2nd], feed_dict=feed_dict))
                )

        return self.sess.run(
            emb, feed_dict={NodeA: self.adj_mat[0:1, :], NodeB: self.adj_mat[1:, :]}
        )
