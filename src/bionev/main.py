...
def parse_args():
    ...
    parser.add_argument("--output", help="Output graph embedding file", required=True)
    parser.add_argument(
        "--task",
        choices=["none", "link-prediction", "node-classification"],
        default="none",
    ...
    parser.add_argument(
        "--label-file", default="", help="The label file for node classification"
    )
    parser.add_argument(
        "--negative-ratio", default=5, type=int, help="the negative ratio of LINE"
    )
    parser.add_argument(
        "--weighted", type=bool, default=False, help="Treat graph as weighted"
    )
    parser.add_argument(
        "--directed", type=bool, default=False, help="Treat graph as directed"
    )
    parser.add_argument(
        "--order",
        default=2,
    ...
    parser.add_argument(
        "--alpha", default=0.3, type=float, help="alhpa is a hyperparameter in SDNE"
    )
    parser.add_argument(
        "--beta", default=0, type=float, help="beta is a hyperparameter in SDNE"
    )
    parser.add_argument(
    ...
    parser.add_argument("--bs", default=200, type=int, help="batch size of SDNE")
    parser.add_argument(
        "--encoder-list",
        default="[1000, 128]",
        type=str,
    ...
    parser.add_argument(
        "--OPT1", default=True, type=bool, help="optimization 1 for struc2vec"
    )
    parser.add_argument(
        "--OPT2", default=True, type=bool, help="optimization 2 for struc2vec"
    )
    parser.add_argument(
        "--OPT3", default=True, type=bool, help="optimization 3 for struc2vec"
    )
    parser.add_argument(
        "--until-layer",
        type=int,
    ...
    parser.add_argument(
        "--dropout", default=0, type=float, help="Dropout rate (1 - keep probability)."
    )
    parser.add_argument(
        "--hidden", default=32, type=int, help="Number of units in hidden layer."
    )
    parser.add_argument(
        "--gae_model_selection",
        default="gcn_ae",
...
def main(args):
    ...
    if args.task == "link-prediction":
        ...
        result = LinkPrediction(
            embedding_look_up, G, G_train, testing_pos_edges, args.seed
        )
        ...
        result = NodeClassification(
            embedding_look_up, node_list, labels, args.testingratio, args.seed
        )
        eval_time = time.time() - time1
        print("Prediction Task Time: %.2f s" % eval_time)
    else:
