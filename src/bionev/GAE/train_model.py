...
from bionev.GAE.preprocessing import (
    construct_feed_dict,
    preprocess_graph,
    sparse_to_tuple,
)
...
class gae_model(object):
    ...
    def save_embeddings(self, output, node_list):
        ...
        for idx in range(emb.shape[0]):
            fout.write(
                "{} {}\n".format(
                    node_list[idx], " ".join([str(x) for x in emb[idx, :]])
                )
            )
        fout.close()
    ...
    def train(self, adj):
                ...
                opt = OptimizerAE(
                    ...
                    labels=tf.reshape(
                        tf.sparse_tensor_to_dense(
                            self.placeholders["adj_orig"], validate_indices=False
                        ),
                        [-1],
                    ),
        ...
        for epoch in range(self.epochs):
            ...
            self.feed_dict = construct_feed_dict(
                adj_norm, adj_label, features, self.placeholders
            )
            self.feed_dict.update({self.placeholders["dropout"]: self.dropout})
            # Run single weight update
            outs = self.sess.run(
                [opt.opt_op, opt.cost, opt.accuracy], feed_dict=self.feed_dict
            )
