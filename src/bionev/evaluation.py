...
def LinkPrediction(
    embedding_look_up, original_graph, train_graph, test_pos_edges, seed
):
    random.seed(seed)
    ...
    print(
        f"AUC-ROC: {auc_roc:.3f}, AUC-PR: {auc_pr:.3f}, Accuracy: {accuracy:.3f}, F1: {f1:.3f}"
    )
    print("#" * 50)
...
def NodeClassification(embedding_look_up, node_list, labels, testing_ratio, seed):
    ...
    X_train, y_train, X_test, y_test = split_train_test_classify(
        embedding_look_up, node_list, labels, testing_ratio=testing_ratio, seed=seed
    )
    ...
    print(
        f"Accuracy: {accuracy:.3f}, Micro-F1: {micro_f1:.3f}, Macro-F1: {macro_f1:.3f}"
    )
    print("#" * 50)
